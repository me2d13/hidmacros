{ 14/02/2007 08:31:38 (GMT+0:00) > [Akadamia] checked in   }
{ 12/02/2007 10:25:20 (GMT+0:00) > [Akadamia] checked out /}
{ 12/02/2007 10:24:35 (GMT+0:00) > [Akadamia] checked in   }
{ 12/02/2007 10:23:52 (GMT+0:00) > [Akadamia] checked out /}
...
uses
  ...
  ActnList, ComCtrls, SimConnectRT, StdActns;
const
...
type
  ...
  TGroupId = (GroupMenu);
  TEventID = (EventMenuOne, EventMenuTwo,EventMenuOneSubOne,EventMenuOneSubTwo);
    ...
    SIMCONNECT_RECV_ID_EVENT:
      begin
        ...
        case TEventId(evt^.uEventID) of
          EventMenuOne, EventMenuOneSubOne, EventMenuOneSubTwo:
            begin
              Memo.Lines.Add(Format('Menu item one selected %d',
                [evt^.dwData]));
              Inc(MenuUseCount);
              // after the second click remove the FIRST sub item
              if MenuUseCount = 2 then
                SimConnect_MenuDeleteSubItem(hSimConnect,Ord(EventMenuOne), Ord(EventMenuOneSubOne));
              // then the second one
              if MenuUseCount = 3 then
                SimConnect_MenuDeleteSubItem(hSimConnect,Ord(EventMenuOne), Ord(EventMenuOneSubTwo));
              ...
              SimConnect_RequestResponseTimes(hSimConnect, 5, Responses[0]);
              for I := 0 to 4 do
                Memo.Lines.Add(Format('%d: %6.5f',[I,Responses[I]]));
            end;
          EventMenuTwo:
            begin
              Inc(MenuUseCount);
              Memo.Lines.Add(Format('Menu item two selected %d',
                [evt^.dwData]));
              if FileName <> '' then
                SimConnect_FlightPlanLoad(hSimConnect,PChar(FileName));
...
procedure TMenuItemsForm.FileOpenAccept(Sender: TObject);
begin
  FileName := ExtractFileName(FileOpen.Dialog.FileName);
end;
...
begin
  ...
  begin
    ...
    hr := SimConnect_MenuAddSubItem(hSimConnect,Ord(EventMenuOne),'SubStuff 1',Ord(EventMenuOneSubOne),6789);
    hr := SimConnect_MenuAddSubItem(hSimConnect,Ord(EventMenuOne),'SubStuff 2',Ord(EventMenuOneSubTwo),9768);
