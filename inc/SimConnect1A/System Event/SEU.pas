{ 25/05/2007 10:53:56 (GMT+1:00) > [Akadamia] checked in   }
{ 25/05/2007 10:53:38 (GMT+1:00) > [Akadamia] checked out /}
{ 14/02/2007 08:35:39 (GMT+0:00) > [Akadamia] checked in   }
{-----------------------------------------------------------------------------
 Unit Name: SEU
 Author:    ken.adam
 Date:      06-Feb-2007
 Purpose:   Translation of System Event example to Delphi
...
type
  ...
  TEventID = (EventFlightLoad);

  // The form
  TSystemEventForm = class(TForm)
...
var
  SystemEventForm       : TSystemEventForm;
...
procedure MyDispatchProc(pData: PSimConnectRecv; cbData: DWORD; pContext:
  Pointer); stdcall;
begin
  SystemEventForm.DispatchHandler(pData, cbData, pContext);
end;
...
procedure TSystemEventForm.DispatchHandler(pData: PSimConnectRecv; cbData:
  DWORD;
  pContext: Pointer);
var
  Evt               : PSimConnectRecvEventFileName;
...
begin
  ...
  case TSimConnectRecvId(pData^.dwID) of
    ...
    SIMCONNECT_RECV_ID_EVENT_FILENAME:
      begin
        evt := PSimConnectRecvEventFileName(pData);
        case TEventId(evt^.uEventID) of
          EventFlightLoad:
           Memo.Lines.Add(Format('New Flight Loaded: %s',[evt^.szFileName]));
        end;
      end;
...
procedure TSystemEventForm.FormCloseQuery(Sender: TObject;
...
procedure TSystemEventForm.FormCreate(Sender: TObject);
begin
...
procedure TSystemEventForm.SimConnectMessage(var Message: TMessage);
begin
...
{-----------------------------------------------------------------------------
  ...
  requests and hooks the system event "FlightLoaded".
  Author:    ken.adam
  Date:      11-Jan-2007
  Arguments:
    Sender: TObject
...
procedure TSystemEventForm.StartEventExecute(Sender: TObject);
var
  hr                : HRESULT;
begin
  ...
  begin
    ...
    hr := SimConnect_SubscribeToSystemEvent(hSimConnect, Ord(EventFlightLoad), 'FlightLoaded');
    StartEvent.Enabled := False;
    StartPoll.Enabled := False;
  end;
end;
...
{-----------------------------------------------------------------------------
  ...
  requests and hooks the system event "FlightLoaded", and then loops indefinitely
  on CallDispatch.
  Author:    ken.adam
  Date:      11-Jan-2007
  Arguments:
...
procedure TSystemEventForm.StartPollExecute(Sender: TObject);
var
  hr                : HRESULT;
begin
