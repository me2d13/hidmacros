{ 25/05/2007 10:40:46 (GMT+1:00) > [Akadamia] checked in   }
{ 14/02/2007 08:33:36 (GMT+0:00) > [Akadamia] checked in   }
{ 14/02/2007 08:33:23 (GMT+0:00) > [Akadamia] checked out /}
{ 12/02/2007 10:16:21 (GMT+0:00) > [Akadamia] checked in   }
{ 08/02/2007 14:08:50 (GMT+0:00) > [Akadamia] checked in   }
{-----------------------------------------------------------------------------
 Unit Name: RKU
 Author:    ken.adam
 Date:      05-Feb-2007
 Purpose:   Translation of Reserved Key example to Delphi
...
type
  ...
  TEventID = (EventReserveRequest);

  // The form
  TReservedKeyForm = class(TForm)
...
var
  ReservedKeyForm       : TReservedKeyForm;
...
procedure MyDispatchProc(pData: PSimConnectRecv; cbData: DWORD; pContext:
  Pointer); stdcall;
begin
  ReservedKeyForm.DispatchHandler(pData, cbData, pContext);
end;
...
procedure TReservedKeyForm.DispatchHandler(pData: PSimConnectRecv; cbData:
  DWORD;
  pContext: Pointer);
var
  OpenData          : PSimConnectRecvOpen;
  rkey : PSimConnectRecvReservedKey;
...
begin
  ...
  case TSimConnectRecvId(pData^.dwID) of
    ...
    begin
      ...
      if TEventID(evt^.uEventID) = EventReserveRequest then
        Memo.Lines.Add('Event: Reserved Key')
      else
        Memo.Lines.Add(Format('Event : %d',[evt^.uEventID]));
    end;
    SIMCONNECT_RECV_ID_RESERVED_KEY:
      begin
        rkey := PSimConnectRecvReservedKey(pData);
        Memo.Lines.Add(Format('Reserved Key: %s  %s',
          [rkey^.szChoiceReserved, rkey^.szReservedKey]));
      end;
...
procedure TReservedKeyForm.FormCloseQuery(Sender: TObject;
...
procedure TReservedKeyForm.FormCreate(Sender: TObject);
begin
...
procedure TReservedKeyForm.SimConnectMessage(var Message: TMessage);
begin
...
procedure TReservedKeyForm.StartEventExecute(Sender: TObject);
var
  hr                : HRESULT;
begin
  ...
  begin
    StatusBar.Panels[0].Text := 'Connected';
		...
		hr := SimConnect_MapClientEventToSimEvent(hSimConnect, Ord(EventReserveRequest));
 		// Use the private event to request a reserved key
    hr := SimConnect_RequestReservedKey(hSimConnect, Ord(EventReserveRequest), 'q', 'a', 'z');
    StartEvent.Enabled := False;
    StartPoll.Enabled := False;
  end;
end;
...
procedure TReservedKeyForm.StartPollExecute(Sender: TObject);
var
  hr                : HRESULT;
begin
